<div style=" font-size: 10pt;">
    <table class="table table-stripped" id="demirbas_grupTable" style=" font-size: 10pt; margin: 0; padding: 0;" width="100%">
        ...
        <?php foreach ($gruplar as $item) { ?>
        <tr>
            <td>
                <div onclick="demirbas_grup_tuttur(<?php echo $item->id . ',' . $demirbas_id ?>)" class="btn btn-xs btn-success"> EKLE</div>
            </td>
            <td><?php echo $item->grup_isim ?></td>
            <td><?php echo $demirbas->sayYaz(100, $item->grup_aciklama) ?></td>
        </tr>
        <?php } ?>
        </tbody>
    </table>
    <div onclick="demirbas_gruplar()" class="btn btn-primary " style="float: left;">GRUPLARI DÜZENLE</div>
